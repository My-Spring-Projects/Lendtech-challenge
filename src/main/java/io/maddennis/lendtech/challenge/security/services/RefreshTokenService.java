...
import io.maddennis.lendtech.challenge.repositories.RefreshTokenRepository;
...
public class RefreshTokenService {
    ...
    @Value("${maddennis.app.jwtRefreshExpirationMs}")
    ...
    public RefreshToken verifyExpiration(RefreshToken token) {
        if (token.getExpiryDate().compareTo(Instant.now()) < 0) {
            ...
            throw new TokenRefreshException(token.getToken(), "Refresh token was expired. SignIn again");
        }
